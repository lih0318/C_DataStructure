...
LinkedList* createLinkedList()
{
	...
	if (pReturn != NULL) {
		...
		printf("Error, Memory allocate createLinkedList()\n");
		return NULL;
	}
		...
		if (position >= 0
			...
			if (pNewNode != NULL) {
				*pNewNode = element;
				...
				printf("Error, Memory allocate addLLElement()\n");
				return ret;
			}
		}
		else {
			printf("Error, position index-[%d], addLLElement()\n",
				position);
		}
	}
...
int removeLLElement(LinkedList* pList, int position)
{
	...
	if (pList != NULL) {
		...
		if (position >= 0 && position < arrayCount)
		{
			...
			printf("Error, position index-[%d] removeLLElement()\n", position);
		}
	}
